...
class ChatService:
    ...
    def _create_chat_graph(self):
        ...
        async def system_prompt_injection_node(state: ChatState):
            ...
            try:
                ...
                if system_prompt and not has_system_message:
                    ...
                    logger.warning("No system prompt to inject")
        ...
        async def llm_processing_node(state: ChatState):
            ...
            try:
                ...
                logger.info(f"Processing with model: {request_data['model']}, mode: {mode}")
                
                response = await llm_chain.ainvoke({"messages": messages})
                
                response_content = response.content if hasattr(response, 'content') else str(response)
                logger.info(f"LLM response generated: {len(response_content)} chars")
    ...
    def _generate_system_prompt(self, request_data: Dict[str, Any], construct_data: Optional[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            if construct_data:
                ...
                # TODO: Provide based system prompt altho we will never reach this part
    ...
    async def process_chat_request(
        ...
        """Process a chat request through the state graph."""
        try:
            construct_data = None
            if request.construct_id:
                try:
                    construct = await get_construct_by_id(db, request.construct_id)
                    logger.debug(f"Fetching construct with ID: {request.construct_id}")
                    logger.debug("Construct found:", construct)
                    if construct:
                        ...
                        logger.info(f"Loaded construct: {construct_data.get('name')}")
                except Exception as e:
                    logger.warning(f"Error fetching construct {request.construct_id}: {e}")
            ...
            logger.info(f"Current request has {len(langchain_messages)} new messages")
            ...
            try:
                ...
                if existing_state and existing_state.values.get("messages"):
                    ...
                    logger.info(f"No existing conversation found for thread {request.thread_id}")
            except Exception as e:
            ...
            logger.info(f"Starting enhanced chat processing for thread: {request.thread_id}")
